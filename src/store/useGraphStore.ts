    ...
    (set) => ({
      ...
      updateProject: async (id, updates) => {
        ...
        try {
          ...
          set((s) => {
            const current =
              s.projects.find((p) => p.id === id) || s.currentProject;
            if (!current) return {};
          ...
          if (current) {
            set((s) => ({
              ...
              currentProject:
                s.currentProject?.id === id ? prev : s.currentProject,
            }));
          }
      ...
      setNodes: (nodes) => {
        ...
        const isValid = (c: unknown): c is string =>
          typeof c === "string" &&
          !!c.trim() &&
          c !== "null" &&
          c !== "undefined";
        const fixedNodes = nodes.map((n) => {
          ...
          if (!isValid(customColor)) {
            customColor =
              NODE_COLORS[hashString(String(n.id)) % NODE_COLORS.length];
          }
      ...
      addTagToNode: (nodeId, tag) =>
        set((state) => ({
          nodes: state.nodes.map((n) =>
            n.id === nodeId ? { ...n, tags: [...(n.tags || []), tag] } : n,
          ),
          activeNode:
            state.activeNode?.id === nodeId
              ? {
                  ...state.activeNode,
                  tags: [...(state.activeNode.tags || []), tag],
                }
              : state.activeNode,
        })),
        ...
        set((state) => ({
          ...
          activeNode:
            state.activeNode?.id === nodeId
              ? {
                  ...state.activeNode,
                  tags: (state.activeNode.tags || []).filter(
                    (t) => t.id !== tagId,
                  ),
                }
              : state.activeNode,
      ...
      addAttachmentToNode: (nodeId, attachment) =>
        set((state) => ({
          nodes: state.nodes.map((n) =>
            n.id === nodeId
      ...
      removeAttachmentFromNode: (nodeId, attachmentId) =>
        set((state) => ({
          nodes: state.nodes.map((n) =>
            n.id === nodeId
              ? {
          ...
          activeNode:
            state.activeNode?.id === nodeId
              ? {
                  ...state.activeNode,
                  attachments: (state.activeNode.attachments || []).filter(
                    (a) => a.id !== attachmentId,
                  ),
                }
              : state.activeNode,
      ...
      setActiveNode: (node) =>
        set({ activeNode: node, isEditorOpen: node !== null }),
      ...
      setConnectionPickerActive: (active) =>
        set({ isConnectionPickerActive: active }),
      ...
      deleteGroup: (id) =>
        set((state) => {
          ...
          const sortedGroups = [...state.groups].sort(
            (a, b) => a.order - b.order,
          );
          ...
          if (state.activeGroupId === id) {
            ...
            else if (idx === 0 && sortedGroups.length > 1)
              fallback = sortedGroups[1].id;
            newActiveGroupId =
              newGroups.find((g) => g.id === fallback)?.id ||
              (newGroups[0]?.id ?? null);
          }
          return {
            groups: newGroups,
      ...
      ensureAtLeastOneGroup: () =>
        set((state) => {
          if (state.groups.length === 0) {
            return {
              groups: [{ id: 0, name: "Default", color: "#808080", order: 0 }],
            };
          }
          return {};
        }),
